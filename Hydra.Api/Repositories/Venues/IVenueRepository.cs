...
{
    Task<(List<Venue> Items, int TotalCount)> GetAllAsync(int skip, int take, Guid? venueTypeId = null, string? name = null, string? location = null, CancellationToken ct = default);
    ...
    Task AddRulesAsync(BookingRules rules, CancellationToken ct = default);
